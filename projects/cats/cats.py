...
def about(topic):
    ...
    def select(paragraphs):
        paragraphs = set(split(lower(remove_punctuation(paragraphs))))
        for x in topic:
            if x in paragraphs:
                return True
        return False
...
def accuracy(typed, reference):
    ...
    for word in typed_words:
        if "\t" in word:
            typed_words.lstrip(word)
    typed_len = len(typed_words)
    reference_len = len(reference_words)
    if typed_len == 0 or reference_len == 0:
        return 0.0
    else:
        paired, i = 0, 0
        while i < typed_len and i < reference_len:
            if typed_words[i] == reference_words[i]:
                paired += 1
            elif typed_words[i] != reference_words[i] and typed_len > reference_len:
                return paired*100/typed_len
            i += 1
        return paired*100/typed_len
    # END PROBLEM 3
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    #     return user_word
    # diff_, word_ = limit, user_word
    # changed = False
    # for word in valid_words:
    #     diff = diff_function(user_word,word,limit)
    #     if diff < diff_ or (diff==diff_ and diff == limit and ~changed):
    #         diff_ = diff
    #         word_ = word
    #         changed = True
...
def shifty_shifts(start, goal, limit):
    ...
    if limit < 0:
        return 0
    elif start_len==0 or goal_len==0:
...
def report_progress(typed, prompt, id, send):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    typed_len, prompt_len = len(typed), len(prompt)
    i = 0
    while i < typed_len:
        if typed[i] != prompt[i]:
            break
        i += 1
    progress = i / prompt_len
...
def time_per_word(times_per_player, words):
    ...
    "*** YOUR CODE HERE ***"
    times = []
    for times_for_one in times_per_player:
        time = []
        for i in range(len(times_for_one)-1):
            time += [times_for_one[i+1] - times_for_one[i]]
        times.append(time)
...
def fastest_words(game):
    ...
    # BEGIN PROBLEM 10
    for w in words:
        min_time = min(time(game,i,w) for i in players)
        for i in players:
            if time(game,i,w) == min_time:
                fastest[i] += [word_at(game,w)]
                break
...
def key_distance_diff(start, goal, limit):
    ...
    if limit < 0:
        ...
        add_diff = key_distance_diff(start,goal[1:],limit-1) + 1  # Fill in these lines
        remove_diff = key_distance_diff(start[1:],goal,limit-1) + 1
        distance = key_distance[(start[0],goal[0])]
        substitute_diff = key_distance_diff(start[1:],goal[1:],limit-1)+distance
        # BEGIN
