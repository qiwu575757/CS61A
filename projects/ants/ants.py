...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        cur_range = 0
        while cur_place is not beehive and cur_range < self.min_range:
            cur_place = cur_place.entrance
            cur_range += 1
        while cur_place is not beehive and cur_range <= self.max_range:
            if cur_place.bees != []:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        for _ in list(self.place.bees):
            _.reduce_armor(amount)

        if self.armor <= amount:
            for _ in list(self.place.bees):
                _.reduce_armor(self.damage)
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        # you may not visit all the elements.his can be prevented by making a copy of the list.
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        ...
        if isinstance(other,ContainerAnt):
            ...
            return self.contained_ant is None
        # END Problem 9
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if self.importor:
            ...
            while cur_place is not None:
                if cur_place.ant and not cur_place.ant.doubled:
                    cur_place.ant.damage *= 2
                    cur_place.ant.doubled = True
                if isinstance(cur_place.ant,ContainerAnt) \
                        and cur_place.ant.contained_ant \
                            and not cur_place.ant.contained_ant.doubled:
                    cur_place.ant.contained_ant.damage *= 2
                    cur_place.ant.contained_ant.doubled = True
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= amount and not self.importor:
            bees_win()
        # END Problem 13
...
def apply_status(status, bee, length):
    ...
    # BEGIN Problem EC
    "*** YOUR CODE HERE ***"
    if status is make_slow:
        for _ in range(length):
            make_slow(status,bee)
    elif status is make_scare:
        for _ in range(length):
            make_scare(status,bee)
    # END Problem EC
