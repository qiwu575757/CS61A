...
def copycat(lst1, lst2):
    ...
    ['c', 'c', 'c']
    """
    def copycat_helper(strlist, intlist, list_sofar):
        if len(strlist) == 0 or len(intlist) == 0 :
            return list_sofar
        if intlist[0] >= 0:
            list_sofar = list_sofar + [strlist[0] for _ in range(intlist[0])]
        else:
            list_sofar = list_sofar[:len(list_sofar)+intlist[0]]
        return copycat_helper(strlist[1:],intlist[1:],list_sofar)
