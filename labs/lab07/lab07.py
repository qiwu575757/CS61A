def insert_into_all(item, nested_list):
    ...
    [[0], [0, 1, 2], [0, 3]]
    """
    # Below is so strange, if the element in list is class, copy() just copy the pointer
    # >>> list = [[]]
    # >>> list_ = list.copy()
    # >>> list_
    # [[]]
...
def subseqs(s):
    ...
    if not s:
        ...
        allsubseqs_but_first = subseqs(s[1:])
        return allsubseqs_but_first + insert_into_all(s[0],allsubseqs_but_first)
...
def make_glookup(class_assignments):
    ...
    def grade_per(work,grade):
        nonlocal total_grade, get_grade
        total_grade = total_grade + class_assignments[work]
        get_grade = get_grade + grade
        return get_grade/total_grade
