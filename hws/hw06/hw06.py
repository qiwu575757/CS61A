...
class VendingMachine:
    ...
    def vend(self):
        ...
        if self.balance < self.price:
            ...
            self.num -= 1
            bal = self.balance - self.price
            self.balance = 0
            return 'Here is your {0} and ${1} change.'.format(self.good,bal)
    ...
    def restock(self,amount):
        self.num += amount
        return 'Current {0} stock: {1}'.format(self.good,self.num)
...
def is_bst(t):
    ...
    if t.is_leaf():
        ...
        if t.label >= bst_max(t.branches[0]) or t.label <  bst_min(t.branches[0]):
            return is_bst(t.branches[0])
        else:
            return False
    elif len(t.branches) == 2 and t.label >= bst_max(t.branches[0]) and t.label <  bst_min(t.branches[1]):
